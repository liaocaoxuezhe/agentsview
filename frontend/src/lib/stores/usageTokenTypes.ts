...
export function canonicalTokenTypes(
  selected: readonly UsageTokenType[],
): UsageTokenType[] {
  const selectedSet = new Set(selected);
  return ALL_TOKEN_TYPES.filter((tokenType) =>
    selectedSet.has(tokenType)
  );
}

export function selectedTokenTypesFromParams(
  ...
  if (
    values.length === 0 ||
    values.some((value) => !TOKEN_TYPE_SET.has(value))
  ) {
...
export function withSelectedTokenTypes(
  ...
  if (
    mode === "token" &&
    canonical.length > 0 &&
    canonical.length < ALL_TOKEN_TYPES.length
  ) {
